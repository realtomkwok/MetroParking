...
class FacilityDataManager: ObservableObject {
	...
	func loadStaticFacilitiesIfNeeded() async {
		...
		// TODO: Get user location for sorting
		let userLocation = getUserLocation()
		let staticFacilities = ParkingFacility.getFacilitiesSortedByDistance(from: userLocation)
		
		print("📦 Inserting \(staticFacilities.count) facilities into SwiftData...")
	...
	func getFacilityStats() async -> FacilityStats {
		...
		let withOccupancyData = allFacilities.filter { $0.isOccupancyCacheValid }
	...
	func getUserLocation() -> (lat: Double, lon: Double) {
		// TODO: Get actual user location from CoreLocation
		return (lat: -33.8688, lon: 151.2093) // Sydney CBD
	}
